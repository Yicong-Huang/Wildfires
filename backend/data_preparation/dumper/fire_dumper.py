...
class FireDumper(DumperBase):
    ...
    sql_create_fire_info_table = 'CREATE TABLE IF NOT EXISTS fire_info (name VARCHAR (40), if_sequence boolean, agency VARCHAR (20), time timestamp, geom_full geometry, geom_1e4 geometry, geom_1e3 geometry, geom_1e2 geometry, center geometry, PRIMARY KEY (name, time))'
    sql_insert_fire_into_history = 'INSERT INTO "fire_crawl_history" (year, name) VALUES (%(year)s, %(firename)s) ON CONFLICT DO NOTHING'
    sql_insert_fire_into_info = 'INSERT INTO "fire_info" (name, if_sequence, agency, time, geom_full, geom_1e4, geom_1e3, geom_1e2, center) VALUES (%(firename)s,%(if_sequence)s,%(agency)s,%(datetime)s,%(geopolygon_full)s,%(geopolygon_large)s,%(geopolygon_medium)s,%(geopolygon_small)s, st_centroid(%(geopolygon_small)s)) ON CONFLICT DO NOTHING'
